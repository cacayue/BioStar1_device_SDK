...
void CInterActiveTestDlg::OnBnClickedBtnDisplay()
{
	...
	{
        result = BS_DisplayCustomInfo( m_Handle, atoi((const char *)m_DisplayTime.GetBuffer(0)), customMsg, ".\\data\\Folder_48.png" );
	}
	else
	{
		result = BS_DisplayCustomInfo( m_Handle, atoi((const char *)m_DisplayTime.GetBuffer(0)), customMsg, NULL );
	}
...
void CInterActiveTestDlg::OnBnClickedBtnPlay()
{
	...
	BS_RET_CODE result = BS_PlaySound( m_Handle, atoi((const char *)m_SoundIndex.GetBuffer(0)) );
	if( result != BS_SUCCESS )
	{
		TCHAR msg[128];
		wsprintf( msg, _T("Can not display custom info: %d"), result );
...
void CInterActiveTestDlg::OnBnClickedBtnWaitKey()
{
	...
	if( m_UseKeyImage )
	{
		result = BS_WaitCustomKeyInput( m_Handle, atoi((const char *)m_WaitKeyTime.GetBuffer(0)), customMsg, ".\\data\\Folder_48.png", key, &keyLen );
	}
	else
	{
		result = BS_WaitCustomKeyInput( m_Handle, atoi((const char *)m_WaitKeyTime.GetBuffer(0)), customMsg, NULL, key, &keyLen );
	}
