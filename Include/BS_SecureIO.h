...
struct BSDoor {
	...
	int useRTE;
	int useDoorSensor;
    unsigned short openEvent;			// not used for BE+, only for BST    
    unsigned char unlockTriggerOption;	//0: Not use, 1: Normal/Admin User, 2: Admin User
...
struct BSInputFunction {
	enum {
		...
		INTERLOCK_ZONE_INPUT = 0x0A,
		LED_GREEN_INPUT = 0x0B, 
		LED_RED_INPUT = 0x0C,
		BUZZER_INPUT = 0x0D,
		ACCESS_GRANTED_INPUT = 0x0E,
		ACCESS_DENIED_INPUT = 0x0F
	};
