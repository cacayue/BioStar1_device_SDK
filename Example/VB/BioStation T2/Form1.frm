...
Private Sub Command15_Click()
    ...
    result = BS_SetDeviceID(handle, biostationID, deviceType)
    'result = BS_SetBiostationID(handle, biostationID)
...
Private Sub Command17_Click()
    ...
    result = BS_ConvertToUTF16("Goog Morning!!" & vbCrLf & "Good Luck!!", privateInfo.greetingMsg(0), BS_MAX_PRIVATE_MSG_LEN)
...
Private Sub Command18_Click()
    ...
    result = BS_GetAllUserInfoBioStation2(handle, userHdr(0), numOfUser)
    If result <> 0 Then
...
Private Sub Command19_Click()
    ...
    result = BS_GetUserInfoBioStation2(handle, userID, userHdr)
    If result <> 0 Then
...
Private Sub Command20_Click()
    ...
    ReDim templateData(384 * 2 * 10)
    
    result = BS_GetUserBioStation2(handle, userID, userHdr, templateData(0))
    If result <> 0 Then
...
Private Sub Command21_Click()
    ...
    userHdr.statusMask = 0
    userHdr.accessGroupMask = &HFFFFFFFF
    result = BS_ConvertToUTF16("Test User", userHdr.userName(0), DS_MAX_NAME_LEN)
    result = BS_ConvertToUTF16("Test Group", userHdr.department(0), DS_MAX_NAME_LEN)
    ...
    For i = 0 To (userHdr.numOfFinger * 2) - 1
        ...
        For j = 0 To 384 - 1
            checksum = checksum + templateData(i * 384 + j)
        Next j
        userHdr.fingerChecksum(Int(i / 2) + 1) = BitShiftRight(checksum, 8)
        userHdr.fingerChecksum(Int(i / 2)) = checksum And &HFF
    Next i

    result = BS_EnrollUserBioStation2(handle, userHdr, templateData(0))
...
Private Sub Command5_Click()
    ...
    result = BS_ReadImage(handle, 0, imageData(0), imageSize)
    If result <> 0 Then
