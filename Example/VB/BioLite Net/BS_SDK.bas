...
Public Type BSWiegandCustomData
    ...
    field(11) As BSWiegandField
    numOfParity As Long
    parity(7) As BSWiegandParity
End Type

Public Type BSWiegandFormatData
    'passThruData As BSWiegandPassThruData '100
    customData As BSWiegandCustomData '236
End Type
...
Public Type BSInputConfig
    ...
    internal(1) As BSInputFunction

    secureIO(3, 3) As BSInputFunction

    slaveTamper As BSInputFunction
    slaveInput(1) As BSInputFunction
...
Public Type BSOutputConfig
    numOfEvent As Long
    outputEvent(183) As BSOutputEvent
    emergencyEvent(3, 3) As BSEMOutputEvent
    reserved(30) As Long
End Type
...
Enum BEConfigValue
    ...
    BIOLITE_CONFIG = &H70
    BIOLITE_CONFIG_SYS_INFO = &H71
    BIOLITE_CONFIG_CARD_READER = &H72
...
Public Type BEOutputPatternBLN
    repeat As Long ' 0: indefinite, -1: don't user
    priority As Long
    arg(2) As Long 'color for LED, frequency for Buzzer, -1 for last
    high(2) As Integer
    low(2) As Integer
End Type


Public Type BELEDBuzzerConfigBLN
    reserved(3) As Long
    ledPattern(49) As BEOutputPatternBLN
    buzzerPattern(49) As BEOutputPatternBLN
    lcdLedPattern(49) As BEOutputPatternBLN
    keypadLedPattern(49) As BEOutputPatternBLN
    signalReserved(49) As Integer
End Type
...
Public Type BETnaEventConfig
    enabled(15) As Byte
    useRelay(15) As Byte
    key(15) As Integer
    eventStr(15, 15) As Byte
End Type
    
    
Public Type BETnaEventExConfig
    fixedTnaIndex As Long
    manualTnaIndex As Long
    timeSchedule(15) As Long
End Type

Public Type BEConfigDataBLN
    ...
    ' operation mode
    opMode(4) As Long
    opModeSchedule(4) As Long
    opDualMode(4) As Byte
    opReserved1(2) As Byte
    opModePerUser As Long
    identificationMode(2) As Long
    identificationModeSchedule(2) As Long
    opReserved2(0) As Long
    ...
    useCheckDuplicate As Byte
    fingerReserved(6) As Long
    ...
    hostID As Long
    slaveIDEx(7) As Long
    slaveType As Long   ' 0 : BST, 1 : BEPL

    ' serial
    serialMode As Long
    serialBaudrate As Long
    userialReserved1 As Byte
    usecureIO As Byte  ' 0x01 - Secure I/O 0, 0x02, 0x04, 0x08
    userialReserved2(5) As Byte
    slaveID As Long  ' 0 for no slave

    reserved1(16) As Long

    ' entrance limit
    minEntryInterval As Long ' 0 for no limit
    numOfEntranceLimit As Long  ' MAX 4
    maxEntry(3) As Long  ' 0 (no limit) ~ 16
    ...
    commandCard(15) As BECommandCard
    ...
    '// LED/Buzzer
    ledBuzzerConfig As BELEDBuzzerConfigBLN

    reserved3(37) As Long

    backlightMode As Long
    soundMode As Long

    tnaEventConfig As BETnaEventConfig
    tnaEventExConfig As BETnaEventExConfig

    reserved4(2) As Long

    cardIdFormatType As Long    '/* CARD_ID_FORMAT_NORMAL, CARD_ID_FORMAT_WIEGAND */
    cardIdByteOrder As Long '/* CARD_ID_MSB, CARD_ID_LSB */
    cardIdBitOrder As Long      '/* CARD_ID_MSB, CARD_ID_LSB */
...
Public Declare Function BS_WriteConfig Lib "BS_SDK" (ByVal handle As Long, ByVal configType As Long, ByVal size As Long, ByRef data As BEConfigDataBLN) As Long
Public Declare Function BS_ReadConfig Lib "BS_SDK" (ByVal handle As Long, ByVal configType As Long, ByRef size As Long, ByRef data As BEConfigDataBLN) As Long
...
Public Declare Function BS_WriteConfigUDP Lib "BS_SDK" (ByVal handle As Long, ByVal targetAddr As Long, ByVal targetID As Long, ByVal configType As Long, ByVal dataSize As Long, ByRef configData As BEConfigDataBLN) As Long
Public Declare Function BS_ReadConfigUDP Lib "BS_SDK" (ByVal handle As Long, ByVal targetAddr As Long, ByVal targetID As Long, ByVal configType As Long, ByRef dataSize As Long, ByRef configData As BEConfigDataBLN) As Long
