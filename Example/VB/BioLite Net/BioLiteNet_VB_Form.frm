VERSION 5.00
Begin VB.Form BioLiteNet 
   Caption         =   "BioLiteNet Sample"
   ClientHeight    =   11370
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   6885
...
Dim userID As Long
Dim configData As BEConfigDataBLN
...
Private Sub Command1_Click()
    result = BS_OpenSocket("192.168.0.27", 1471, handle)
    If result <> 0 Then
    ...
    For i = 0 To 128
        ...
        If deviceTypes(i) = 1 Then
            If deviceIDs(i) = 1059 Then
                result = BS_WriteConfigUDP(internalHandle, readerAddrs(i), deviceIDs(i), BIOLITE_CONFIG, Len(configData), configData)
                If result <> 0 Then
...
Private Sub Command15_Click()
    ...
    For i = 0 To 128
        ...
        If deviceTypes(i) = 1 Then
            If deviceIDs(i) = 1059 Then
                result = BS_ReadConfigUDP(internalHandle, readerAddrs(i), deviceIDs(i), BIOLITE_CONFIG, readSize, configData)
                If result <> 0 Then
...
Private Sub Command2_Click()
    ...
    userHdr.accessGroupMask = &HFFFFFFFE
...
'    Next i
    ...
    retval = lstrcpy(userHdr.password, "1111")
    
    userHdr.password(0) = &H37
    userHdr.password(1) = &H38
    userHdr.password(2) = &H39
    userHdr.password(3) = &H30
    userHdr.password(4) = &H0
...
Private Sub Command3_Click()
    ...
    Text1.SelText = "CardID:" & userHdr.cardID & "    OpMode:" & userHdr.opMode & vbCrLf
...
Private Sub Command5_Click()
    ...
    For i = 384 To 768
        templateData(i) = templateData(i - 384)
    Next i
...
Private Sub Command8_Click()
    ...
    result = BS_ReadConfig(handle, BIOLITE_CONFIG, readSize, configData)
    If result <> 0 Then
        MsgBox "Cannot Read Configuration : " & result
        Exit Sub
    End If
     
    Text1.SelText = "Use DHCP:" & configData.useDHCP & vbCrLf
    Text1.SelText = "IP:" & (configData.ipAddr And &HFF) & "." & ((configData.ipAddr And &HFF00) / (2 ^ 8) And &HFF) & "." & ((configData.ipAddr And &HFF0000) / (2 ^ 16) And &HFF) & "." & ((configData.ipAddr And &HFF000000) / (2 ^ 24) And &HFF) & vbCrLf
    Text1.SelText = "Gateway:" & (configData.gateway And &HFF) & "." & ((configData.gateway And &HFF00) / (2 ^ 8) And &HFF) & "." & ((configData.gateway And &HFF0000) / (2 ^ 16) And &HFF) & "." & ((configData.gateway And &HFF000000) / (2 ^ 24) And &HFF) & vbCrLf
    Text1.SelText = "Subnetmask:" & (configData.subnetMask And &HFF) & "." & ((configData.subnetMask And &HFF00) / (2 ^ 8) And &HFF) & "." & ((configData.subnetMask And &HFF0000) / (2 ^ 16) And &HFF) & "." & ((configData.subnetMask And &HFF000000) / (2 ^ 24) And &HFF) & vbCrLf
    Text1.SelText = "Use Server:" & configData.useServer & "    Server IP:" & (configData.serverIpAddr And &HFF) & "." & ((configData.serverIpAddr And &HFF00) / (2 ^ 8) And &HFF) & "." & ((configData.serverIpAddr And &HFF0000) / (2 ^ 16) And &HFF) & "." & ((configData.serverIpAddr And &HFF000000) / (2 ^ 24) And &HFF) & vbCrLf
    Text1.SelText = "synchTime:" & configData.synchTime & vbCrLf
    
    Text1.SelText = "useWiegandInput:" & configData.useWiegandInput & vbCrLf
    Text1.SelText = "useWiegandOutput:" & configData.useWiegandOutput & vbCrLf
    Text1.SelText = "wiegandIdType:" & configData.wiegandIdType & vbCrLf
    Text1.SelText = "wiegandMode:" & configData.wiegandMode & vbCrLf
    Text1.SelText = "wiegandReaderID:" & configData.wiegandReaderID & vbCrLf
    Text1.SelText = "wiegandConfig.data.customData.idFieldMask:" & configData.wiegandConfig.data.customData.idFieldMask & vbCrLf
    Text1.SelText = "wiegandConfig.header.format:" & configData.wiegandConfig.header.format & vbCrLf
...
Private Sub Command9_Click()
    ...
    configData.useServerMatching = 1
    
    result = BS_WriteConfig(handle, BIOLITE_CONFIG, Len(configData), configData)
    If result <> 0 Then
        MsgBox "Cannot Write Configuration : " & result
        Exit Sub
    End If
